...
def test_blackout_4h_fridge_light_router(
    ...
    assert req.energy_ac_wh == 366
    assert req.continuous_power_ac_w == 189
    assert req.startup_power_w == 789
...
def test_boiler_8h(gas_boiler: ApplianceSpec) -> None:
    ...
    assert req.energy_ac_wh == 480
    assert req.apparent_power_va == 200
...
def test_phones_only_10h(phone: ApplianceSpec) -> None:
    ...
    assert req.energy_dc_wh == 40
    assert req.energy_ac_wh == 0
